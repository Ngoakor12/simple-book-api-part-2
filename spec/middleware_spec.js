...
fdescribe("Books API with Middleware", () => {
  it("should log each request (logging not tested explicitly but status checks logging presence)", async () => {
    const response = await request(app).get("/books");
    expect(response.status).toBe(200);
  });

  it("should return 400 for an invalid book ID format", async () => {
